...
if __name__ == "__main__":
    ...
    parser.add_argument('-i", "--start-frame', help = 'index of first frame', type=int)
    parser.add_argument('-ss', "--sima", help= "data coming from sima", type=int)
    parser.add_argument('-s', "--sima-analysis", help= "needs sima to analyze?", type=int)

    args = parser.parse_args()
    if args.sima ==1:
    	dataset = sima.ImagingDataset.load(args.input)
    	...
    	if args.sima_analysis ==1:
    		vid = vid.T.reshape([vid.shape[-1], 1, frame_shape[0], frame_shape[1], 1])
    		sequences = [sima.Sequence.create('ndarray', vid)]
    		dataset_path = args.input+'.sima'
   ...
   # 	if key==ord('s'):
    ...
    if args.raster == 1:
    	for i in range(len(rois)):
    		plt.scatter(time, np.multiply(spikes[:,i],i+1), marker='.')
    	plt.xlabel('Time (s)')
    	plt.ylabel('cell')
    	plt.axvline(x=90)
    	plt.axvline(x=120)
